...
func main() {
    ...
    for sc.Scan() {
        ...
        // Unescape the query string to prevent special characters from being URL-encoded
        u.RawQuery, err = url.PathUnescape(u.RawQuery)
        if err != nil {
            fmt.Fprintf(os.Stderr, "failed to unescape query string: %s\n", err)
            continue
        }
